...
internal object TlmValidator {
    ...
    val MAX_EXPECTED_PDU_COUNT = 10 * 60 * 60 * 24 * 365 * 3

    // Maximum expected time since boot in beacon telemetry.
    // Given that and a lifetime of ~3 years, any value above this is suspicious.
    val MAX_EXPECTED_SEC_COUNT = 10 * 60 * 60 * 24 * 365 * 3
    ...
    val STORE_NEXT_FRAME_DELTA_MS = 3000
    ...
    fun validate(deviceAddress: String, serviceData: ByteArray, beacon: Beacon) {
        ...
        if (beacon.tlmServiceData == null) {
            ...
            if (Arrays.equals(beacon.tlmServiceData, serviceData)) {
                val err = "TLM service data was identical to recent TLM frame:\n%s"
                        .format(Utils.toHexString(serviceData))
        ...
        if (serviceData.size < MIN_SERVICE_DATA_LEN) {
            val err = "TLM frame too short, needs at least %d bytes, got %d"
                    .format(MIN_SERVICE_DATA_LEN, serviceData.size)
        ...
        val version = buf.get()
        beacon.tlmStatus.version = "0x%02X".format(version)
        if (version != EXPECTED_VERSION) {
            val err = "Bad TLM version, expected 0x%02X, got %02X".format(EXPECTED_VERSION, version)
        ...
        if (!voltage.equals(0)
                && (voltage < MIN_EXPECTED_VOLTAGE || voltage > MAX_EXPECTED_VOLTAGE)) {
            val err = "Expected TLM voltage to be between %d and %d, got %d"
                    .format(MIN_EXPECTED_VOLTAGE, MAX_EXPECTED_VOLTAGE, voltage)
        ...
        val tempFractional = (buf.get().toInt() and 255)
        ...
        if (temp != TEMPERATURE_NOT_SUPPORTED) {
            if (temp < MIN_EXPECTED_TEMP || temp > MAX_EXPECTED_TEMP) {
                val err = "Expected TLM temperature to be between %.2f and %.2f, got %.2f"
                        .format(MIN_EXPECTED_TEMP, MAX_EXPECTED_TEMP, temp)
        ...
        if (advCnt > MAX_EXPECTED_PDU_COUNT) {
            val err = "TLM ADV count %d is higher than expected max of %d"
                    .format(advCnt, MAX_EXPECTED_PDU_COUNT)
            beacon.tlmStatus.errPduCnt = err
            logDeviceError(deviceAddress, err)
        }
        if (previousTlm != null) {
        ...
        val uptime = buf.int
        beacon.tlmStatus.secCnt = "%d (%d days)"
                .format(uptime, TimeUnit.SECONDS.toDays((uptime / 10).toLong()))
        if (uptime <= 0) {
        ...
        if (uptime > MAX_EXPECTED_SEC_COUNT) {
            val err = "TLM time since boot %d is higher than expected max of %d"
                    .format(uptime, MAX_EXPECTED_SEC_COUNT)
            beacon.tlmStatus.errSecCnt = err
            logDeviceError(deviceAddress, err)
        }
        if (previousTlm != null) {
